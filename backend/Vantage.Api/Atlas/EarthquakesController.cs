...
public sealed class EarthquakesController(EarthquakeStore store, EarthquakeSources sources, VantageDbContext db) : ControllerBase
{
    [HttpGet("source")]
    public EarthquakeSourceDto Source() => sources.Active.Metadata;
    [HttpGet]
    public Task<EarthquakeSnapshotDto> Query(CancellationToken cancellationToken) => store.QueryAsync(Source(), cancellationToken);
