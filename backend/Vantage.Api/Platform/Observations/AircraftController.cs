...
public sealed class AircraftController(AircraftStore store, AircraftSources sources, VantageDbContext db, PlatformAccess access) : ControllerBase
{
    [HttpGet("source")]
    public async Task<AircraftSourceDto> Source(CancellationToken ct) { await access.RequireDataAsync(ct); return sources.Active.Metadata; }
    [HttpGet]
        ...
        [FromQuery] int radiusNm = 250, CancellationToken cancellationToken = default)
    {
        ...
        return await store.QueryAsync(sources.Active.Metadata.Id, query.Normalized(), sources.Active.Metadata.ResultLimit, cancellationToken);
    }
