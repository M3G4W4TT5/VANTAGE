...
{
    public string ConnectionId { get; set; } = BuiltinConnections.Earthquakes;
...
public sealed class EarthquakeStore(VantageDbContext db, ObservationValidation validation)
{
    public async Task SaveAsync(string sourceId, EarthquakeFetch fetch, CancellationToken ct,
        string connectionId = BuiltinConnections.Earthquakes, string datasetId = BuiltinConnections.EarthquakeDataset,
        long configurationRevision = 1)
    {
        ...
        var feed = await db.EarthquakeFeeds.SingleOrDefaultAsync(x => x.ConnectionId == connectionId && x.SourceId == sourceId, ct);
        ...
        var current = await db.CurrentEarthquakes.Where(x => x.ConnectionId == connectionId && x.SourceId == sourceId).ToDictionaryAsync(x => x.Id, ct);
        ...
        foreach (var delivery in deliveries)
        {
            ...
            if (!linked.Contains(o.Id)) db.ObservationDeliveries.Add(new() { ConnectionId = connectionId, DatasetId = datasetId,
                ObservationId = o.Id, ConfigurationRevision = configurationRevision, RetrievedAt = fetch.RetrievedAt });
            if (row is null)
            {
                row = new() { ConnectionId = connectionId, Id = o.EntityId, SourceId = sourceId }; db.CurrentEarthquakes.Add(row); current.Add(row.Id, row);
            }
        ...
        if (feed is null) { feed = new() { ConnectionId = connectionId, SourceId = sourceId }; db.EarthquakeFeeds.Add(feed); }
    ...
    public async Task<EarthquakeSnapshotDto> QueryAsync(EarthquakeSourceDto source, CancellationToken ct,
        string connectionId = BuiltinConnections.Earthquakes)
    {
        var cutoff = DateTimeOffset.UtcNow.AddHours(-source.CacheHours);
        var feed = await db.EarthquakeFeeds.AsNoTracking().SingleOrDefaultAsync(x => x.ConnectionId == connectionId && x.SourceId == source.Id && x.RetrievedAt >= cutoff, ct);
        var rows = feed is null ? [] : await db.CurrentEarthquakes.AsNoTracking().Where(x => x.ConnectionId == connectionId && x.SourceId == source.Id && x.InLatestFeed && x.LastSeenAt >= cutoff)
    ...
    public async Task PruneAsync(string sourceId, CancellationToken ct, string connectionId = BuiltinConnections.Earthquakes)
    {
        var cutoff = DateTimeOffset.UtcNow.AddHours(-EarthquakeCachePolicy.RetentionHours);
        var scope = db.CurrentEarthquakes.Where(x => x.ConnectionId == connectionId && x.SourceId == sourceId);
