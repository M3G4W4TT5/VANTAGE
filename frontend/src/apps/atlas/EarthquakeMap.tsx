...
export function EarthquakeMap({ channel, basemapId, mode, setMode, camera, selectedId, select, setCamera, matches }: {
  channel: EarthquakeChannel; basemapId: string; mode: '2d' | '3d'; setMode(mode: '2d' | '3d'): void; camera?: CameraState; selectedId?: string;
  select(id: string, observationId?: string): void; setCamera(camera: CameraState): void; matches(record: EarthquakeRecord): boolean;
}) {
  return <PointMap label="Earthquake map" basemapId={basemapId} mode={mode} setMode={setMode} camera={camera} selectedId={selectedId}
