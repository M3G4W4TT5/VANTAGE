...
> **2026-09-23 ATLAS interaction refinement:** the owner's compact, functional tree/list requirement supersedes earlier spacious row and dock examples below. Layers, Sources and Records use contiguous outliner-style rows, indentation and thin rules rather than per-item cards. Map and List can be shown together. NEXUS manages connections; ATLAS groups configured datasets. The current bottom bar is a timeline placeholder.
...
- **Spacing:** 4, 8, 12, 16, 24, 32px. Forms and inspectors use the standard 12px panel padding. ATLAS operational trees use 4–6px side padding, indentation and contiguous rows; do not inherit form-section gaps.
- **Geometry:** 1px rules; 0–2px radii; 32px general desktop controls; 26–28px ATLAS tree rows with 24px icon targets, expanded to 44px on coarse pointers. Expand a row only for required visible data. Preserve the focus and target rules in §7.
...
│ Layers / Sources │  MAP / GLOBE  │  LIST   │ Inspector    │
│ Category → group │              │ tree    │ (record)     │
│ compact rows     │              │ rows    │              │
├ Timeline (expandable; current capability placeholder) ──────┤
└ Map attribution remains on the canvas ───────────────────────┘
```
...
- **Initial ATLAS view:** show the saved map/list arrangement with a compact Layers / Sources sidebar. Map and List are independent, resizable panes. Open the inspector on record selection. Restore pane visibility, widths and split with the workspace.
...
- **Layers:** compose implemented datasets in category → group rows with a visibility eye and no group-focus control. Category/group visibility hides map and List content together; a record eye in List hides only that map appearance. The hierarchy contains no individual record rows or repeated cards. Categories are Vehicles & satellites; Events & alerts; Places & infrastructure; Environment & measurements; Imagery & overlays; Feeds & reports; My work. Examples do not add providers to the specification.
- **Group controls:** a compact plus button adds a group. Right click a group or press Shift+F10 for Filters, Actions, Settings and confirmed Delete group, in that order. Filters and Actions have separate Save/Discard dialogs; Settings contains Sources, Appearance, Coverage & time and available previous-view recovery without horizontal tab scrolling. Offer a legend after a 500ms hover or immediately on keyboard focus. Selecting a map/List record establishes the current group for record-specific controls; drawing order remains independent.
- **Collection:** showing a dormant configured group explicitly starts its pane demand. After activation, the group eye changes only Map/List presentation; hidden active groups retain their cached snapshot and live subscription for immediate re-show without recreating the map. Hidden demand still consumes provider and transport capacity; equivalent requests share upstream work, while removing the group or closing the pane releases its demand. NEXUS connection enablement and other panes are unaffected. These controls do not imply recording, operator polling or a change to retained observations.
- **Sources:** show a condensed name/status list without per-row buttons. Right click a source or press Shift+F10 for a context menu with **Configure in NEXUS**. Add and delete ATLAS presentation groups in Layers and select one or more compatible configured datasets there. NEXUS remains the connection manager.
...
- **ATLAS controls:** Map and List are independent toolbar toggles; both can be open with a draggable, keyboard-resizable split. Map projection is labelled **2D / 3D**, with an equally wide and tall **Terrain** menu button beneath the pair. Clicking it again or outside closes its options. A compact map tool button holds place search and the currently supported area action for a selected aircraft record. Sidebar and inspector retain drag/keyboard resizing. The bottom bar expands only the unavailable timeline placeholder for now.
...
- **Records:** List is the full queried-area category → group → record tree. It shows all visible groups and uses compact semantic tables at available width; narrow splits keep record selection and map-eye controls visible, with full facts in inspection. Each group can hide or show all of its map appearances; after hiding all, individual record eyes can show selected records. Count cached appearances honestly and label cache/coverage limits. Unknown locations remain selectable. Preserve source provenance and stable selection. Richer media and raster results remain future capability work.
- **Views:** keep the current Map and List panes independent. Future media, recording and replay may extend the shared dock after their data contracts and workflows exist.
...
- Prefer contiguous compact rows, indentation, thin rules, tabs and small metric grids. Operational trees should read like outliners, not cards. Use cards only when an object needs several related attributes outside a scan list.
...
- [ ] Check combined layers, category/drawing order, record selection, group visibility, participation, overlap picking, shared legends and grouped/mixed/domain results with honest counts.
